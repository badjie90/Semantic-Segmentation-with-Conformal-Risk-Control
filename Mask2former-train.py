"""
Mask2Former DEL design-time pipeline.
...
class Config:
    ...
    base_model: str = "facebook/mask2former-swin-tiny-ade-semantic"
    num_classes: int = 3
    ...
    def resolved(self) -> "Config":
        ...
        return Config(**{
            **asdict(self),
            ...
            "output_dir": self.output_dir or os.path.join(root, "TRAINING", "MASK2FORMER", "DEL", "Training-outputs"),
            "split_dir": self.split_dir or os.path.join(root, "TRAINING", "MASK2FORMER", "DEL", "splits"),
        })
...
def load_mask_resized(mask_path: str, size: int, num_classes: int) -> np.ndarray:
    mask = Image.open(mask_path).resize((size, size), resample=Image.NEAREST)
    arr = np.array(mask)
    if arr.ndim == 3:
        arr = arr[:, :, 0]
...
class SegDataset(Dataset):
    def __init__(self, items: List[Dict[str, str]], processor: Mask2FormerImageProcessor, cfg: Config):
        self.items = items
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        labels = load_mask_resized(self.items[idx]["mask"], self.cfg.image_size, self.cfg.num_classes)
        ...
        pixel_mask = encoded.get(
            "pixel_mask",
            torch.ones((1, self.cfg.image_size, self.cfg.image_size), dtype=torch.long),
        ).squeeze(0)
        return {
            "pixel_values": pixel_values,
            "pixel_mask": pixel_mask.to(torch.long),
...
def make_loader(items: List[Dict[str, str]], processor: Mask2FormerImageProcessor, cfg: Config, shuffle: bool) -> DataLoader:
    return DataLoader(
...
def build_model(cfg: Config, pretrained: bool = True) -> Mask2FormerForUniversalSegmentation:
    ...
    model_cfg = Mask2FormerConfig(num_labels=cfg.num_classes, id2label=id2label, label2id=label2id)
    return Mask2FormerForUniversalSegmentation(model_cfg)


def outputs_to_semantic_probs(outputs, num_classes: int, target_hw: Tuple[int, int], eps: float = 1e-6) -> torch.Tensor:
    class_probs = F.softmax(outputs.class_queries_logits, dim=-1)[..., :num_classes]
    mask_probs = outputs.masks_queries_logits.sigmoid()
    sem_probs = torch.einsum("bqc,bqhw->bchw", class_probs, mask_probs)
    sem_probs = F.interpolate(sem_probs, size=target_hw, mode="bilinear", align_corners=False)
    sem_probs = sem_probs.clamp_min(0.0)
    return sem_probs / (sem_probs.sum(dim=1, keepdim=True) + eps)


def probs_to_logits(probs: torch.Tensor, eps: float) -> torch.Tensor:
...
def configure_trainable_parameters(model: nn.Module, epoch_index: int, cfg: Config) -> str:
    set_requires_grad(model.parameters(), True)
    if epoch_index < cfg.freeze_backbone_epochs:
        for name, p in model.named_parameters():
            if "class_predictor" not in name and "mask_embedder" not in name and "mask_predictor" not in name:
...
@torch.no_grad()
def eval_path_iou(model: nn.Module, loader: DataLoader, cfg: Config, device: torch.device) -> float:
    model.eval()
    tp = fp = fn = 0
    for batch in loader:
        x = batch["pixel_values"].to(device, non_blocking=True)
        pm = batch["pixel_mask"].to(device, non_blocking=True)
        y = batch["labels"].to(device, non_blocking=True)
        outputs = model(pixel_values=x, pixel_mask=pm)
        probs = outputs_to_semantic_probs(outputs, cfg.num_classes, y.shape[-2:], cfg.eps)
...
def train_baseline(model: nn.Module, train_loader: DataLoader, calib_loader: DataLoader, cfg: Config, device: torch.device) -> str:
    ce_loss = nn.CrossEntropyLoss()
    optimizer = AdamW(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay)
    scheduler = CosineAnnealingLR(optimizer, T_max=cfg.baseline_epochs)
    best_iou = -1.0
    best_path = os.path.join(cfg.output_dir, "baseline_theta0_best_mask2former.pth")

    for epoch in range(cfg.baseline_epochs):
        model.train()
        total = 0.0
        for batch in tqdm(train_loader, desc=f"[baseline CE][{epoch + 1}/{cfg.baseline_epochs}]"):
            x = batch["pixel_values"].to(device, non_blocking=True)
            pm = batch["pixel_mask"].to(device, non_blocking=True)
            y = batch["labels"].to(device, non_blocking=True)
            outputs = model(pixel_values=x, pixel_mask=pm)
            probs = outputs_to_semantic_probs(outputs, cfg.num_classes, y.shape[-2:], cfg.eps)
            loss = ce_loss(probs_to_logits(probs, cfg.eps), y)
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            optimizer.step()
            total += float(loss.item())
        scheduler.step()
        path_iou = eval_path_iou(model, calib_loader, cfg, device)
    ...
    torch.save(model.state_dict(), os.path.join(cfg.output_dir, "baseline_theta0_final_mask2former.pth"))
    return best_path


def train_edl(model: nn.Module, train_loader: DataLoader, calib_loader: DataLoader, theta0: Dict[str, torch.Tensor], cfg: Config, device: torch.device) -> str:
    ce_loss = nn.CrossEntropyLoss()
    best_iou = -1.0
    best_path = os.path.join(cfg.output_dir, "evidential_theta_best_mask2former.pth")
    optimizer = None
    scheduler = None
    current_mode = None
    for epoch in range(cfg.edl_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"[EDL fine-tune][{epoch + 1}/{cfg.edl_epochs}]"):
            x = batch["pixel_values"].to(device, non_blocking=True)
            pm = batch["pixel_mask"].to(device, non_blocking=True)
            y = batch["labels"].to(device, non_blocking=True)
            outputs = model(pixel_values=x, pixel_mask=pm)
            probs = outputs_to_semantic_probs(outputs, cfg.num_classes, y.shape[-2:], cfg.eps)
            logits = probs_to_logits(probs, cfg.eps)
            ce = ce_loss(logits, y)
        ...
        if path_iou > best_iou:
            best_iou = path_iou
            torch.save(model.state_dict(), best_path)
    torch.save(model.state_dict(), os.path.join(cfg.output_dir, "evidential_theta_final_mask2former.pth"))
    return best_path


class Mask2FormerSemanticExport(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, pixel_mask: torch.Tensor) -> torch.Tensor:
        ...
        return outputs_to_semantic_probs(
            outputs,
...
def export_onnx(model: nn.Module, cfg: Config, device: torch.device, out_path: str) -> str:
    model.eval()
    wrapper = Mask2FormerSemanticExport(model, cfg).to(device).eval()
    dummy_pixels = torch.randn(1, 3, cfg.image_size, cfg.image_size, device=device)
    dummy_mask = torch.ones(1, cfg.image_size, cfg.image_size, dtype=torch.long, device=device)
    torch.onnx.export(
        wrapper,
        (dummy_pixels, dummy_mask),
        out_path,
        export_params=True,
        opset_version=17,
        do_constant_folding=True,
        input_names=["pixel_values", "pixel_mask"],
        output_names=["semantic_probs"],
        dynamic_axes={
            "pixel_values": {0: "batch"},
            "pixel_mask": {0: "batch"},
            "semantic_probs": {0: "batch"},
        },
    )
    return out_path
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Methodology-matched Mask2Former DEL design-time training.")
...
def run_smoke_test(cfg: Config, device: torch.device) -> None:
    print("Running Mask2Former training smoke test...")
    smoke_cfg = Config(**{**asdict(cfg), "image_size": 64})
    model = build_model(smoke_cfg, pretrained=False).to(device)
    model.eval()
    x = torch.randn(1, 3, smoke_cfg.image_size, smoke_cfg.image_size, device=device)
    pm = torch.ones(1, smoke_cfg.image_size, smoke_cfg.image_size, dtype=torch.long, device=device)
    y = torch.randint(0, smoke_cfg.num_classes, (1, smoke_cfg.image_size, smoke_cfg.image_size), device=device)
    with torch.no_grad():
        outputs = model(pixel_values=x, pixel_mask=pm)
        probs = outputs_to_semantic_probs(outputs, smoke_cfg.num_classes, y.shape[-2:], smoke_cfg.eps)
    logits = probs_to_logits(probs, smoke_cfg.eps)
    ...
    print(
        "Smoke test OK: "
        f"probs={tuple(probs.shape)}, ce={float(ce.item()):.4f}, "
...
def main() -> None:
    ...
    processor = Mask2FormerImageProcessor.from_pretrained(cfg.base_model)
    ...
    onnx_path = export_onnx(model, cfg, device, os.path.join(cfg.output_dir, "evidential_theta_best_mask2former_del.onnx"))
    write_json(os.path.join(cfg.output_dir, "training_summary.json"), {
        "methodology": "Mask2Former CE baseline followed by EDL fine-tuning with masked KL, beta annealing, L2-SP, staged unfreezing, and split export.",
        "config": asdict(cfg),
