"""
Mask2Former DEL runtime evaluation and conformal selective prediction.

Implements manuscript runtime:
- Mask2Former query outputs -> per-pixel class probability maps.
- Dirichlet alpha = softplus(log(p)) + 1.
...
class Config:
    ...
    def resolved(self) -> "Config":
        ...
        return Config(**{
            **asdict(self),
            "split_json": self.split_json or os.path.join(root, "TRAINING", "MASK2FORMER", "DEL", "splits", "split.json"),
            "model_path": self.model_path or os.path.join(root, "TRAINING", "MASK2FORMER", "DEL", "Training-outputs", "evidential_theta_best_mask2former.pth"),
            "output_dir": self.output_dir or os.path.join(root, "TRAINING", "MASK2FORMER", "DEL", "Test-outputs"),
        })
...
def make_loader(items: List[Dict[str, str]], processor: Mask2FormerImageProcessor, cfg: Config) -> DataLoader:
    return DataLoader(
...
def build_model(cfg: Config, pretrained: bool = True) -> Mask2FormerForUniversalSegmentation:
    ...
    return Mask2FormerForUniversalSegmentation(Mask2FormerConfig(num_labels=cfg.num_classes, id2label=id2label, label2id=label2id))


def outputs_to_semantic_probs(outputs, num_classes: int, target_hw: Tuple[int, int], eps: float) -> torch.Tensor:
...
@torch.no_grad()
def dirichlet_outputs_from_probs(sem_probs: torch.Tensor, eps: float) -> Dict[str, torch.Tensor]:
    logits = torch.log(sem_probs.clamp_min(eps))
...
@torch.no_grad()
def collect_pixels(model, loader: DataLoader, cfg: Config, device: torch.device, desc: str) -> Dict[str, np.ndarray]:
    ...
    for batch in tqdm(loader, desc=desc):
        ...
        sem_probs = outputs_to_semantic_probs(outputs, cfg.num_classes, y.shape[-2:], cfg.eps)
        out = dirichlet_outputs_from_probs(sem_probs, cfg.eps)
...
def save_core_metrics(data: Dict[str, np.ndarray], cfg: Config, metric_dir: str) -> None:
    ...
    correct = pred == y
    with open(os.path.join(metric_dir, "mask2former_del_test_metrics.csv"), "w", newline="") as f:
        w = csv.writer(f)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Mask2Former DEL runtime evaluation.")
    p.add_argument("--project-root", default=Config.project_root)
    p.add_argument("--split-json", default="path to your test json split file")
...
def run_smoke_test(cfg: Config, device: torch.device) -> None:
    print("Running Mask2Former runtime smoke test...")
    ...
    with torch.no_grad():
        ...
        outputs = model(pixel_values=x, pixel_mask=pm)
        sem_probs = outputs_to_semantic_probs(outputs, smoke_cfg.num_classes, (smoke_cfg.image_size, smoke_cfg.image_size), smoke_cfg.eps)
        out = dirichlet_outputs_from_probs(sem_probs, smoke_cfg.eps)
    n = 128
    ...
    for alpha in cfg.alpha_values:
        ...
        for score_name in cfg.score_names:
            thresholds[score_name] = class_conditional_thresholds(
                calib_data["pred"], calib_data["y"], calib_data[score_name], cfg.num_classes, alpha
            )
